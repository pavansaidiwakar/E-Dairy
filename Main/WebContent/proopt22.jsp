...
<form  target="_blank"  action="cartnew.jsp" method="GET" id="form1" >
	 <%
		...
		if(flag2==0){
		%><ul style="display:inline;list-style-type:none;"><h5 id="<%= rs1.getString("type").toUpperCase()%>"> <%=rs1.getString("type").toUpperCase()%></h5>
		<%}if(flag2!=0 && !previous.equals(rs1.getString("type"))){ %>
		</ul><ul style="list-style-type:none;"><br><br><br><br><br><h5 id="<%= rs1.getString("type").toUpperCase()%>"><%=rs1.getString("type").toUpperCase()%></h5><br>
		<%}%> 
		<li style="display:inline-block;">
		<div style="width:280px;height:350px;float:left;border:3px solid #ccffcc; margin-left:15px;margin-top:15px;text-align:center;"> 
		  <img style="width:250px;height:200px;" src=<%= rs1.getString("image_link")%> alt=""/>
				<div style="background-color: #ccffcc;height:140px;width:278px;margin-left:0px;margin-top:7px;color:black;">
					<br>
					...
					<select  style="width:150px; " name="<%= rs1.getString("pid") %>" >
					<%for(int p=0;;p++)
		...
		<%} %>
		<button style:"width:30px; align:center;" type="submit" form="form1"/>View Cart</button>
		</div>	
		</form>
